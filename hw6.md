...
## 📝 程式日誌紀錄系統 (Logging System)

本專案建置了完整的日誌紀錄系統，能自動將模型訓練步驟與使用者在儀表板上的操作紀錄同時輸出至控制台（Console）與本地日誌檔案中：

*   **`solve_50_startups.log`**：當您執行機器學習建模腳本時產生。紀錄內容包含資料集加載狀況、訓練/測試集切分尺寸、5 種機器學習模型之預測指標（R²、MAE、RMSE）對比、最佳模型判定，以及前向特徵選擇（SFS）的執行進度。
*   **`streamlit_app.log`**：當您運行 Streamlit 儀表板時產生。紀錄內容包含網頁初始化/刷新事件，以及每次使用者滑動滑桿並點擊預測時的輸入參數（研發、行政、行銷支出、地區）與對應的利潤預估結果。

這兩個日誌檔案皆已納入 `.gitignore` 中忽略，確保您的 GitHub 儲存庫保持乾淨，不含有本地端執行日誌。
